  ...
  initStream: function() {
    ...
    this.stream.on('status', function(status) {
      switch(status) {
        ...
        case 420:
        default:
          that.emit('stream-stop', {name:"Twitter w/ HTTP "+status});
        break;
      }
    });
    this.stream.start();
      ...
      switch(command) {
        ...
        case 'topic':
          if(isHelp) {
            ...
            this._room && this._room.setTopic(_(o).each(cleanToken).join(' '), this);
          }
        break;
        ...
        case 'stop':
          if(isHelp) {
            ...
            this._room && this._room.killStream(this);
          }
        break;

        case 'help':
        default:
          responseMsg = 'This is a Twitter-enabled chatroom. The topic is shown at the top of the page. ';
          responseMsg += 'All chatroom participants see the same stream of live twitter activity matching the current topic.';
          responseMsg += '<br/>';
          responseMsg += 'Commands you can try: <kbd>/topic, /stop, /nick</kbd>.<br/>';
